...
class Process:
    def __init__(self, pid, priority, burst_time):
        """Initialize a process with parameters."""
        self.pid = pid
...
class MultilevelQueueScheduler:
    def __init__(self, num_queues, aging_threshold):
        """Initialize a multilevel queue scheduler with a number of queues and an aging threshold."""
        # Initialize queues as empty lists
    ...
    def run_scheduler(self):
        ...
        # Continue scheduling until all queues are empty
        while any(queue for queue in self.queues):
            for i in range(len(self.queues)):
                if self.queues[i]:
                    ...
                    current_process.wait_time += self.current_time

                    # Aging mechanism
                    if self.current_time != 0 and self.current_time % self.aging_threshold == 0:
                        self.boost_priority(current_process)

                    # Move the process back to the queue if not finished
                    if current_process.remaining_time > 0:
                        self.queues[i].append(current_process)

        print(f"Total time elapsed: {self.current_time}")
...
class SchedulerApp:
    def __init__(self, master):
        ...
        self.aging_threshold_var = tk.IntVar(value=5)
    ...
    def create_widgets(self):
        ...
        # Aging Threshold entry
        ttk.Label(self.master, text="Aging Threshold:").grid(row=4, column=1, sticky=tk.W, pady=5)
        self.aging_threshold_entry = ttk.Entry(self.master, textvariable=self.aging_threshold_var)
        self.aging_threshold_entry.grid(row=5, column=1, pady=5)

        # Run button
        ttk.Button(self.master, text="Run Scheduler", command=self.run_scheduler).grid(row=6, column=0, columnspan=2, pady=10)
    ...
    def run_scheduler(self):
        ...
        try:
            ...
            aging_threshold = self.aging_threshold_var.get()
            ...
            # Initialize scheduler and set parameters
            scheduler = MultilevelQueueScheduler(num_queues=num_queues, aging_threshold=aging_threshold)
            ...
            # Run the scheduler and get total time elapsed
